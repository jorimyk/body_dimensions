...
from bd_api.utils import CommonUtils, MeasurementUtils
...
def addNewMeasurement(userId, d):
    ...
    if (d.get('fatTotal') or d.get('bodyMass') or d.get('fatVisceral')) and not d.get('weight'):
        ...
        data = Measurements( \
        userId = userId, \
        measurementDate = convertFromISODate(d.get('measurementDate')), \
        height = d.get('height'), \
        weight = d.get('weight'), \
        ...
        session.commit()
        data = data.serialize
        data['measurementDate'] = convertToISODate(data['measurementDate'])
...
def getMeasurements(userId):
    ...
    if data:
        ...
        for index in data:
            index['measurementDate'] = convertToISODate(index['measurementDate'])
        return jsonify(data)
    else:
        return ('', 204)
    ...
    if data:
        if d:
            ...
            if 'measurementDate' in d and validateDate(userId, 'measurementDate', d.get('measurementDate')):
                validRequest = True
                data.measurementDate = convertFromISODate(d.get('measurementDate'))
